...
namespace ToDoList.WebApp.Controllers
{
    ...
    public class ListController : Controller
    {
        ...
        //GET:
        public ViewResult Index(SortListViewModel vm, int page = 1, string sortOrder = "asc", string SearchString = "")
        {
            ...
            {
                Lists = lists,
                PagingInfo = new PagingInfo() { CurrentPage = page, TotalItems = itemsPerPage }
            });
        }
        ...
        {
            var listToView = repository.TodoListRepository.GetAll().Include(y => y.Tasks).FirstOrDefault(x => x.Id == id);//GetByCondition(x => x.Id == id).Include(y => y.Tasks).Select(x=>x).First();
